...
def is_dir_writeable(dir_path: Union[str, Path]) -> bool:
    ...
    """
    # ``TemporaryFile`` can spend minutes retrying on some Windows setups
    # (notably when the profile directory is monitored by security software).
    # Importing Ultralytics then appears to hang before a model is even loaded.
    # For this read-only capability check, the OS access flags are sufficient;
    # the actual settings write still reports a normal OSError if it fails.
    path = Path(dir_path)
...
_config_dir = os.getenv('YOLO_CONFIG_DIR')
USER_CONFIG_DIR = Path(_config_dir) if _config_dir else get_user_config_dir()  # Ultralytics settings dir
