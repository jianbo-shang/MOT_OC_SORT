...
try:
...
except ModuleNotFoundError:  # setuptools >= 81 no longer guarantees pkg_resources
    ...
    class _PkgCompat:
        ...
        def parse_requirements(requirements):
            lines = requirements if not isinstance(requirements, str) else requirements.splitlines()
            return (Requirement(line.strip()) for line in lines
        ...
        def require(requirement):
            ...
            try:
                ...
                raise _PkgCompat.DistributionNotFound(parsed.name) from exc
            if parsed.specifier and installed not in parsed.specifier:
                raise _PkgCompat.VersionConflict(f'{parsed.name} {installed}')
